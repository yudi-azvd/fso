...
void* read_data(void* _id) {
  ...
  while (1) {
    printf(C_BLUE "R%0ld requesting readers_mutex (%d readers)\n", id, readers_in_shared_mem);
    sem_wait(&readers_in_shared_mem_mutex);
    printf(C_BLUE "R%0ld got readers_mutex (%d readers)\n", id, readers_in_shared_mem);
    ++readers_in_shared_mem;
    if (readers_in_shared_mem == 1)
      sem_wait(&mutex);
    printf(C_BLUE "R%0ld releasing readers_mutex (%d readers)\n", id, readers_in_shared_mem);
    sem_post(&readers_in_shared_mem_mutex);

    printf(C_BLUE "R%0ld read %d (%d readers)\n", id, shared_data, readers_in_shared_mem);
    sleep_random(2);

    printf(C_BLUE "R%0ld out\n", id);
...
void* write_data(void* _id) {
  ...
  while (1) {
    printf(C_RED "%sW%0ld requesting shared_data_mutex (%d readers)\n", TABS, id, readers_in_shared_mem);
    sem_wait(&mutex);
    printf(C_RED "%sW%0ld got shared_data_mutex (%d readers)\n", TABS, id, readers_in_shared_mem);

    if (readers_in_shared_mem != 0) {
      printf(C_RED "%sERROR > W %0ld: %d readers in shared mem\n", TABS, id, readers_in_shared_mem);
      exit(1);
    }

    shared_data = rand() % 256;
    printf(C_RED "%sW%0ld wrote %d (%d readers)\n", TABS, id, shared_data, readers_in_shared_mem);
    sleep_random(2); 
  
    printf(C_RED "%sW%0ld leaving\n", TABS, id);
    sem_post(&mutex);
    printf(C_RED "%sW%0ld out\n", TABS, id);
